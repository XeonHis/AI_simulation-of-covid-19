...
def run_dqn(_env, _approximator, _approximator_target, _batch_size, _max_episode, _memory_size, _update_time, _gamma,
    ...
    writer = SummaryWriter(
        'runs_cartpole/model_episode={}_lr={}_bsize={}_utime={}'.format(_max_episode, _learning_rate, _batch_size,
                                                                        _update_time))
    best_reward = -np.inf
    learn_step = 0
    epsilon = 1
    ...
    for i_episode in range(_max_episode):
        ...
        while not done:
            ...
            if len(_memory) >= _batch_size:  # Start to learn
                ...
                writer.add_scalar('loss', loss.item(), learn_step)
            ...
            if done:
                ...
                if episode_reward >= best_reward:
                    torch.save(_approximator.state_dict(),
                               'cartpole/lr={}_bsize={}_utime={}.pkl'.format(_learning_rate, _batch_size,
                                                                             _update_time))
                    f = open('cartpole_actions.txt', 'w+')
        ...
        # if i_episode % 10 == 0:
        #     torch.save(_approximator.state_dict(),
        #                'model/test/lr={}_bsize={}_utime={}_each.pkl'.format(_learning_rate, _batch_size,
        #                                                                     _update_time))
...
def DQN_agent(env, train_mode, batch_size=32, learning_rate=0.02, max_episode=1000, memory_size=2000, update_time=100,
              gamma=0.9, path='cartpole/lr=0.01_bsize=32_utime=100.pkl'):
...
if __name__ == '__main__':
    ...
    DQN_agent(env, train_mode=True, batch_size=32, learning_rate=0.001, max_episode=1000, memory_size=2 ** 19,
              update_time=10, gamma=0.99, path='cartpole/lr=0.01_bsize=256_utime=200.pkl')
